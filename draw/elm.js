...
Elm.Draw.make = function (_elm) {
   ...
   var mouseDrag$$ = A3($Signal$Extra.sampleWhen,
   $Mouse.isDown,
   {ctor: "_Tuple2",_0: 0,_1: 0},
   $Mouse.position);
   var mouseDrag$ = A3($Signal$Extra.keepWhen,
   $Mouse.isDown,
   {ctor: "_Tuple2",_0: 0,_1: 0},
   $Mouse.position);
   var mouseDrag = A2($Signal.map,
   $Basics.fst,
   A3($Signal.filter,
   $Basics.snd,
   {ctor: "_Tuple2",_0: {ctor: "_Tuple2",_0: 0,_1: 0},_1: false},
   A3($Signal.map2,
   F2(function (v0,v1) {
      return {ctor: "_Tuple2",_0: v0,_1: v1};
   }),
   ...
   var mouseAction = function () {
      var click = A2($Signal.map,Click,clicks);
      var drag = A2($Signal.map,Move,mouseDrag);
      return A2($Signal.merge,drag,click);
   }();
   var initialModel = _U.list([]);
   var state = A3($Signal.foldp,update,initialModel,mouseAction);
   var h = 1000;
   var w = 1000;
   var radius = 2;
   var color = $Color.black;
   var scene = function (model) {
      var draw = function (_p1) {
         var _p2 = _p1;
         return A2($Graphics$Collage.move,
         {ctor: "_Tuple2"
         ,_0: $Basics.toFloat(_p2._0) - $Basics.toFloat(w) / 2
         ,_1: $Basics.toFloat(h) / 2 - $Basics.toFloat(_p2._1)},
      ...
      1000,
      1000,
      A2($List.map,draw,model));
   };
   var main = A2($Signal.map,scene,state);
